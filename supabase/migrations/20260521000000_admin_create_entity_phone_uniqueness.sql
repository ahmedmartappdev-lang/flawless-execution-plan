-- Add phone uniqueness checks to admin_create_vendor / admin_create_admin /
-- admin_create_delivery_partner. Two real-world consequences of allowing
-- duplicate phones:
--   1) send-otp validates role membership by phone — duplicate rows make
--      .maybeSingle() error out, the JS code treats that as "not found",
--      so the legitimate user is told "number not registered".
--   2) post-OAuth-signup linkage (claim_role_by_email) and any future
--      phone-based linkage become non-deterministic across duplicate rows.
--
-- Email uniqueness was already enforced. Adding the same shape for phone.
-- Function bodies are otherwise unchanged from 20260509000000.
...
BEGIN
  ...
  END IF;
  IF v_phone <> '' AND EXISTS (SELECT 1 FROM public.admins WHERE phone = v_phone) THEN
    RAISE EXCEPTION 'an admin with this phone number already exists';
  END IF;
  ...
  BEGIN
    PERFORM public.log_admin_action('admin_create_admin', 'admin', v_id,
...
BEGIN
  ...
  END IF;
  IF v_phone <> '' AND EXISTS (SELECT 1 FROM public.delivery_partners WHERE phone = v_phone) THEN
    RAISE EXCEPTION 'a delivery partner with this phone number already exists';
  END IF;
  ...
  BEGIN
    PERFORM public.log_admin_action('admin_create_delivery_partner', 'delivery_partner', v_id,
