-- Honor admin_selling_price override in place_customer_order_with_credit.
--
-- Bug: the catalog-authoritative RPC introduced in 20260422040000 reads
-- products.selling_price (the vendor's price) and ignores the admin
-- override stored in products.admin_selling_price. Result: every order
-- is silently recorded at the lower vendor price even when the admin
-- has set a higher override that the customer was quoted in the cart.
--
-- Fix: read the effective price as
--   COALESCE(NULLIF(admin_selling_price, 0), selling_price)
-- which matches the convention used in src/stores/cartStore.ts:64
-- (treat NULL or 0 as "no override, use vendor's price"). Body of the
-- function is otherwise byte-identical to the previous version — same
-- service-area gate, same stock locking, same credit handling, same
-- order_items insert structure.
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- effective price: admin override if set (non-null and non-zero), else vendor price
