-- Enforce delivery within configured service areas at the RPC layer.
-- Customer order placement now hard-rejects out-of-zone deliveries.
-- Admin's admin_create_order is intentionally NOT changed.
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM service_areas WHERE is_active) INTO v_any_active;
  IF NOT v_any_active THEN
    -- No zones configured = serve everywhere (preserves pre-config behavior).
    RETURN TRUE;
  END IF;

  -- Haversine: km between point and each circle's centre, compare to radius_km.
  SELECT EXISTS(
    SELECT 1
    FROM service_areas
    WHERE is_active
      AND 2 * 6371 * asin(sqrt(
            sin(radians(p_lat - center_latitude) / 2) ^ 2
          + cos(radians(center_latitude)) * cos(radians(p_lat))
            * sin(radians(p_lng - center_longitude) / 2) ^ 2
        )) <= radius_km
  ) INTO v_match;
...
GRANT EXECUTE ON FUNCTION public.is_location_serviceable(NUMERIC, NUMERIC)
  TO authenticated, service_role;

-- Add the gate to place_customer_order_with_credit. Body otherwise unchanged
-- from the catalog-authoritative version (20260422040000).
...
BEGIN
  ...
  -- Service-area gate (hard reject — only the server enforces this for real)
