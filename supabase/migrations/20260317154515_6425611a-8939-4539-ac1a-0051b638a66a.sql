...
CREATE OR REPLACE FUNCTION public.place_customer_order_with_credit(
  p_vendor_groups jsonb,        -- array of { vendor_id, items: [{ product_id, product_snapshot, quantity, unit_price, mrp, discount_amount, total_price }] }
...
BEGIN
  ...
  -- Lock and validate credit balance
  IF p_credit_used > 0 THEN
    ...
    IF p_credit_used > v_current_balance THEN
  ...
  LOOP
    ...
    -- Only first vendor group gets fees
    IF v_idx = 0 THEN
    ...
    -- Credit allocation for this order
