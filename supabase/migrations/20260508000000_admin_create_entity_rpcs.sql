-- SECURITY DEFINER RPCs for admin-side entity creation.
-- Same pattern as record_cash_settlement and claim_role_by_email:
-- admin gate via is_admin(), insert as service-role-equivalent so RLS
-- can't silently block, return the new row id, log to admin_audit_log.

-- ============================================================
-- 1. Add a vendor (pre-registration; user_id left NULL until they OAuth-sign-up
--    and claim_role_by_email links the row).
-- ============================================================
...
REVOKE ALL ON FUNCTION public.admin_create_vendor(JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_create_vendor(JSONB) TO authenticated;

-- ============================================================
-- 2. Add an admin (team member)
-- ============================================================
...
REVOKE ALL ON FUNCTION public.admin_create_admin(JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_create_admin(JSONB) TO authenticated;

-- ============================================================
-- 3. Add a delivery partner (agent)
-- ============================================================
