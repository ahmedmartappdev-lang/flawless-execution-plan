...
const ProductDetailsPage: React.FC = () => {
  ...
  const ProductCard = ({ p }: { p: Product }) => {
    ...
    return (
      <div className="min-w-[200px] max-w-[200px] border border-[#eee] rounded-[12px] p-[12px] relative bg-white flex flex-col h-full hover:shadow-md transition-shadow">
        {discount > 0 && <div className="absolute top-0 left-[10px] bg-[#4a75e6] text-white text-[10px] px-[6px] py-[4px] font-extrabold rounded-b-[4px] z-10">{discount}% OFF</div>}
        ...
        <div className="bg-[#f8f8f8] text-[9px] font-extrabold px-[6px] py-[3px] rounded-[4px] mb-[8px] inline-flex items-center gap-1 w-fit"><Clock className="w-3 h-3" /> 16 MINS</div>
        <div className="text-[13px] font-semibold leading-[1.3] h-[34px] overflow-hidden mb-[4px] text-[#1f1f1f] line-clamp-2" title={p.name}>{p.name}</div>
        <div className="text-[12px] text-[#666] mb-[15px]">{p.unit_value} {p.unit_type}</div>
        <div className="flex justify-between items-center mt-auto">
          <span className="text-[13px] font-bold">₹{p.selling_price}</span>
          {qty === 0 ? (
            <button className="border border-[#0c831f] bg-[#f7fff9] text-[#0c831f] px-[18px] py-[6px] rounded-[6px] font-bold text-[13px] hover:bg-[#0c831f] hover:text-white transition-colors" onClick={() => handleAddToCart(p)}>ADD</button>
          ) : (
            <div className="flex items-center bg-[#0c831f] text-white rounded-[6px] h-[32px]">
              <button className="px-2 h-full font-bold hover:bg-[#096e1a] rounded-l-[6px]" onClick={() => decrementQuantity(p.id)}>-</button>
              <span className="px-2 text-[13px] font-bold min-w-[20px] text-center">{qty}</span>
              <button className="px-2 h-full font-bold hover:bg-[#096e1a] rounded-r-[6px]" onClick={() => incrementQuantity(p.id)}>+</button>
            </div>
          )}
        </div>
      </div>
  ...
  if (isLoading) return <div className="min-h-screen bg-white flex items-center justify-center"><Skeleton className="w-full h-full max-w-4xl rounded-xl" /></div>;
  if (!product) return <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4"><h2 className="text-2xl font-bold mb-4">Product Not Found</h2><Button onClick={() => navigate('/')}>Go Home</Button></div>;
      ...
      <main className="max-w-[1280px] mx-auto py-10 px-4 md:px-10">
        ...
        <section className="flex flex-col md:flex-row gap-10 md:gap-16 pb-16 border-b border-[#eeeeee]">
          ...
          <div className="flex-1 pt-5">
            <div className="text-[12px] text-[#666] mb-2.5 flex items-center flex-wrap gap-1">
              <span className="cursor-pointer hover:text-[#0c831f]" onClick={() => navigate('/')}>Home</span> <ChevronRight className="w-3 h-3" />
              <span className="cursor-pointer hover:text-[#0c831f]" onClick={() => navigate(`/category/${product.category?.slug}`)}>{product.category?.name || 'Category'}</span> <ChevronRight className="w-3 h-3" />
              <span className="text-[#1f1f1f] line-clamp-1">{product.name}</span>
            </div>
            
            {product.brand && <div className="text-[13px] font-bold text-[#666] mb-1 uppercase tracking-wide">{product.brand}</div>}
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="bg-[#e7f6e8] text-[#0c831f] px-2 py-0.5 rounded flex items-center gap-1 text-xs font-bold border border-[#e7f6e8]">
                <Star className="w-3 h-3 fill-[#0c831f] text-[#0c831f]" />{product.rating || '4.5'}
              </div>
              <span className="text-xs text-[#666] font-medium underline cursor-pointer">{product.total_reviews || 0} reviews</span>
            </div>
              ...
              <div className="border border-[#0c831f] bg-[#f7fff9] rounded-[12px] px-5 py-3 cursor-pointer min-w-[100px] relative">
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  {product.mrp > product.selling_price && (
                    <div className="relative"><span className="text-[12px] text-[#666]">₹{product.mrp}</span><motion.div initial={{ width: 0 }} animate={{ width: '100%' }} transition={{ duration: 0.4, delay: 0.2 }} className="absolute top-1/2 left-0 h-[1px] bg-red-500 -translate-y-1/2" /></div>
                  )}
                </div>
            ...
            <div className="flex justify-between items-end mb-8 border-b border-[#f0f0f0] pb-8">
              <div><span className="text-[14px] font-extrabold block">₹{product.selling_price}</span><span className="text-[10px] text-[#666] font-medium">(Inclusive of all taxes)</span></div>
              {currentQty === 0 ? (
                <button className="bg-[#0c831f] text-white border-none px-9 py-3 rounded-[8px] font-bold text-[16px] cursor-pointer hover:bg-[#096e1a] transition-colors" onClick={() => handleAddToCart(product)}>Add to cart</button>
              ) : (
                <div className="flex items-center bg-[#0c831f] text-white rounded-[8px] h-[48px]">
                  <button className="px-4 h-full font-bold hover:bg-[#096e1a] rounded-l-[8px] text-lg" onClick={() => decrementQuantity(product.id)}>-</button>
                  <span className="px-4 text-[16px] font-bold min-w-[40px] text-center">{currentQty}</span>
                  <button className="px-4 h-full font-bold hover:bg-[#096e1a] rounded-r-[8px] text-lg" onClick={() => incrementQuantity(product.id)}>+</button>
                </div>
              )}
            </div>
            
            {product.description && <div className="mb-10"><div className="text-[16px] font-bold mb-3 text-[#1f1f1f]">Product Description</div><p className="text-[13px] text-[#666] leading-[1.6]">{product.description}</p></div>}
            
            <div className="mt-6">
              <div className="text-[14px] font-bold mb-4 text-[#333]">Why shop from Ahmad Mart?</div>
        ...
        <section className="py-10">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
            {similarLoading ? [...Array(5)].map((_, i) => <Skeleton key={i} className="min-w-[200px] h-[300px] rounded-xl" />) : similarProducts?.map(p => <ProductCard key={p.id} p={p} />)}
          </div>
        </section>
        ...
        <section className="py-10">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
            {alsoBoughtLoading ? [...Array(5)].map((_, i) => <Skeleton key={i} className="min-w-[200px] h-[300px] rounded-xl" />) : alsoBoughtProducts?.map(p => <ProductCard key={p.id} p={p} />)}
          </div>
        </section>
